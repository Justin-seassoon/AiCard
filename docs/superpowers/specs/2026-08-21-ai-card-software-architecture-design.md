...
| 游客知识库 vkb | **1.0 实现（2026-09）**：游客提问翻译成日语后撞 VKB，命中则「嘀嘀」提示服务人员，OK 确认后答案反向翻译成游客语言播放 |
...
|---|---|---|---|---|
| 游客知识库 vkb（SW-VKB 全 9 项 P0） | 0804 §4.1 P0「翻译 + 游客知识库并行匹配、员工 OK 确认后播放」 | **已由「1.0 后置」变更为「1.0 实现」**（2026-09）：游客提问翻译成日语后撞 VKB，命中提示服务人员，OK 确认后答案反向翻译播放 | §0.2、§1.2 | 已实现（2026-09） |

> ✅ **已变更（2026-09）**：vkb 从「1.0 后置」改为「1.0 实现」，云端已实现（游客 FAQ + 服务人员确认流程），不再偏离 PRD，无需更新 PRD scope。
